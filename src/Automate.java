...
/*
 * static Automate conversionAfnAfd(Automate afn) {
 * 
 * // On crée un afd vide qu'on remplira pendant la conversion
 * Automate afd = new Automate();
 * 
 * // Pour chaque état de l'afn, on regroupe les transitions avec les mêmes
 * symboles et on fusionne les états d'arrivé en super-état
 * for (Etat e : afn.etats) {
 * 
 * // On regroupe les transitions qui lisent le même symbole
 * HashMap<String, ArrayList<Transition>> symbolesLus = new HashMap<>();
 ...
 * }
 * 
 * // On crée un super-état à chaque fois qu'il y a plus de deux transitions
 ...
 * 
 * for (Transition t : transitions) {
 * somme += t.arrivEtat.exp;
 * }
 * 
 * Etat se = new Etat(somme, 2);
 * 
 * afn.addEtat(se);
 * }
 * }
